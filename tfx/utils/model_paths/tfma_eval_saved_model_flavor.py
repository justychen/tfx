...
def lookup_model_paths(export_dir_base: Text) -> List[ModelPathTuple]:
  ...
  Returns:
    A list of (model_path, timestamp) namedtuples.
  """
  result = []
  for timestamp in tf.io.gfile.listdir(export_dir_base):
    ...
    result.append(ModelPathTuple(model_path, timestamp))
  return result


def lookup_only_model_path(export_dir_base: Text) -> ModelPathTuple:
  ...
  Returns:
    (model_path, timestamp) namedtuples.
  """

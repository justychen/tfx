...
`{model_base_path}/{model_name}/{version}` to be correctly recognized from the
model server. We call this a *TFS-flavored model path*.

Example:

```
gs://your_bucket_name/foo/bar/  # A `model_base_path`
...
_TF_SERVING_MODEL_PATH_PATTERN = re.compile(
    r'^(?P<model_base_path>.*)/(?P<model_name>[^/]+)/(?P<version>\d+)')
...
def parse_model_path(
  ...
  """
  match = _TF_SERVING_MODEL_PATH_PATTERN.match(model_path)
  if not match:
    raise ValueError('{} does not match TF Serving flavor.'.format(model_path))

  match_dict = match.groupdict()
  model_base_path = match_dict['model_base_path']
  model_name = match_dict['model_name']

...
class TFMAEvalSavedModelFlavorTest(tf.test.TestCase):
  ...
  def testLookupModelPaths_ForSingleModel(self):
    ...
    model_path, timestamp = model_paths[0]
  ...
  def testLookupModelPaths_ForMultipleModels(self):
    ...
    model_path, timestamp = mp1
    self.assertEqual(os.path.relpath(model_path, export_dir_base), '1582798459')
    self.assertEqual(timestamp, 1582798459)

    model_path, timestamp = mp2
    self.assertEqual(os.path.relpath(model_path, export_dir_base), '1582858365')
    self.assertEqual(timestamp, 1582858365)

  def testLookupOnlyModelPath(self):
    export_dir_base = self._SetupSingleModel()

    model_path, timestamp = tfma_eval_saved_model_flavor.lookup_only_model_path(
        export_dir_base=export_dir_base)

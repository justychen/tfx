...
ModelPathTuple = NamedTuple('ModelPathTuple', [
    ('model_path', Text),
    ('exporter_name', Text),
    ('timestamp', int)
])
...
def lookup_model_paths(export_path: Text) -> List[ModelPathTuple]:
  ...
  Returns:
    A list of (model_path, exporter_name, timestamp) namedtuples.
  """
  ...
  for exporter_name in tf.io.gfile.listdir(export_sub_dir):
    ...
    for timestamp in tf.io.gfile.listdir(model_sub_dir):
      ...
      result.append(ModelPathTuple(model_path, exporter_name, timestamp))

  return result


def lookup_only_model_path(export_path: Text):
  ...
  Returns:
    (model_path, exporter_name, timestamp) namedtuples.
  """
